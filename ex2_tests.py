...
if s3:
    ...
    assert np.all(kernel_factory(3)-kernel3<1e-10), "Kernel is " + str(kernel_factory(3)) + ", should be " + str(kernel3)
    assert np.all(kernel_factory(11)-kernel11<1e-10), "Kernel is " + str(kernel_factory(11)) + ", should be " + str(kernel11)
    for i in range(50):
        kernel_size = i*2 + 1
        kernel = kernel_factory(kernel_size)
